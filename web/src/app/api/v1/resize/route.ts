...
const resizeSchema = z.object({
  ...
  keep_aspect_ratio: z.enum(['true', 'false']).optional().default('true'),
});
...
export async function POST(request: Request): Promise<Response> {
  ...
  const parsed = resizeSchema.safeParse({
    ...
    keep_aspect_ratio: formData.get('keep_aspect_ratio') ?? undefined,
  });
  if (!parsed.success) {
    return NextResponse.json<ApiResponse<never>>(
      {
        success: false,
        error: {
          message: parsed.data ? 'Invalid dimensions' : parsed.error.errors[0].message,
  ...
  const { width, height, keep_aspect_ratio } = parsed.data;
  const keepAspect = keep_aspect_ratio === 'true';
  ...
  try {
    const { output, durationMs } = await imageService.resize(buffer, width, height, keepAspect);
    ...
    return NextResponse.json<ApiResponse<never>>(
      { success: false, error: { message: 'Image resize failed', code: 'INTERNAL_ERROR' } },
      { status: 500 }
    );
  }
}
